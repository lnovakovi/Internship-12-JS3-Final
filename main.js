...
<form>
<input id="name" type ="text" name="username" placeholder="Username" minlength="5" maxlength="10" required/>
<input id="password" type="password" name="password" placeholder="Password" minlength="5" maxlength="10" required/>
<input id="password-confirm" type="password" name="password-conf" placeholder="Confirm password" minlength="5" maxlength="10" required/>
<input type="submit" id="btnSubmit" name="signUp-button" value="Sign up"/>
<p> Min length of username and password is 5 char , max 10 char</p>
</form>`;
...
<input type="submit" id="btnSubmitLogin" name="signUp-button"  value="Sign in"/>
</form>`;
...
function getPosts(userid) {
  fetch(
    `https://jsonplaceholder.typicode.com/users/1/posts?userId=${userid + 1}`
  )
    .then(data => {
...
function createPost(id) {
  modal.innerHTML += `<div class="modal-content">
    ...
    <textarea id="post" rows="10" cols="50" placeholder ="Enter text" required></textarea>
  ...
  modal.querySelector("#btnSubmitPost").addEventListener("click", function() {
    event.preventDefault();
    if (!title.checkValidity() || !post.checkValidity()) {
